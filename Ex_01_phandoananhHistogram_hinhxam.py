...
def ChuyendoiAnhmauxambangppLumninace(imgPIL):
    ...
    for x in range(width):
        for y in range(height):
            ...
            if (gray >= T1 and gray <= T2) :
                Luminance.putpixel((x,y), (gray, gray, gray))
                his[gray] += 1
            else :
                Luminance.putpixel((x,y), (255, 255, 255))
                
    return Luminance, his
...
def VebieudoHistogram(his):
    ...
    # đặt giá trị y là 8000 để dễ quan sát như hình gốc
    plt.ylim(top=8000)
...
#chuyển sang mức xám
HinhXamPIL, his = ChuyendoiAnhmauxambangppLumninace(imgPIL)
