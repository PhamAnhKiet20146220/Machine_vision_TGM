...
class Remove_Object():
    def __init__(self, master, segmentModel, rcnn_transforms, deepfill, img_path, img_path2):
        ...
        open_list_button = Button(root, text='Mở danh sách nội thất',command=self.open_object_list_window)
        open_list_button.pack()

        # Tạo nút xóa đối tượng
        delete_button = Button(root, text='xóa nội thất mới thêm vào từ danh sách',command=self.delete_object)
        delete_button.pack()
